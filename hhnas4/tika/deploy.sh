...
Examples:
  ./deploy.sh nas-host
  ./deploy.sh nas-host /volume1/docker/homelab/nas-host/tika
  ./deploy.sh nas-host --update-env
USAGE
}

TARGET_HOST="nas-host.internal.example"
TARGET_DIR="/volume1/docker/homelab/nas-host/tika"
UPDATE_ENV=0
...
echo "[deploy] tika deployed. Verify with:"
...
echo "         ssh ${TARGET_HOST} \"curl -sS -m 6 -i http://127.0.0.1:\$(grep '^TIKA_PORT=' '${REMOTE_ENV_FILE}' | cut -d= -f2)/version | sed -n '1,12p'\""
