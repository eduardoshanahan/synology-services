...
FreshRSS stack on `nas-host`, intended to sit behind DSM reverse proxy.
...
`127.0.0.1:${FRESHRSS_HOST_PORT:-8095}` on `nas-host`.
...
```bash
ssh nas-host "cd /volume1/docker/homelab/nas-host/freshrss && docker compose ps"
ssh nas-host "curl -sSI http://127.0.0.1:8095/ | head -n 5"
```
