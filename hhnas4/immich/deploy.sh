...
Examples:
  ...
  ./deploy.sh nas-host.internal.example /volume1/docker/homelab/nas-host/immich
  ./deploy.sh nas-host.internal.example --update-env
USAGE
}

TARGET_HOST="nas-host.internal.example"
TARGET_DIR="/volume1/docker/homelab/nas-host/immich"
UPDATE_ENV=0
...
UPLOAD_DIR="$(ssh "${TARGET_HOST}" "grep '^UPLOAD_LOCATION=' '${REMOTE_ENV_FILE}' | cut -d= -f2-")"
DB_HOSTNAME="$(ssh "${TARGET_HOST}" "grep '^DB_HOSTNAME=' '${REMOTE_ENV_FILE}' | cut -d= -f2-")"
REDIS_HOSTNAME="$(ssh "${TARGET_HOST}" "grep '^REDIS_HOSTNAME=' '${REMOTE_ENV_FILE}' | cut -d= -f2-")"

if [[ -z "${UPLOAD_DIR}" || -z "${DB_HOSTNAME}" || -z "${REDIS_HOSTNAME}" ]]; then
	echo "[deploy] required settings are missing in ${REMOTE_ENV_FILE} (UPLOAD_LOCATION/DB_HOSTNAME/REDIS_HOSTNAME)" >&2
	exit 1
fi

ssh "${TARGET_HOST}" "mkdir -p '${UPLOAD_DIR}'"
...
echo "[deploy] immich deployed. Verify with:"
...
echo "         ssh ${TARGET_HOST} \"curl -sS -m 6 -i http://127.0.0.1:\$(grep '^IMMICH_HOST_PORT=' '${REMOTE_ENV_FILE}' | cut -d= -f2) | sed -n '1,12p'\""
