...
On `hhnas4`, the active reverse proxy entry was observed in:
...
nix develop -c ./hhnas4/qbittorrent/deploy.sh hhnas4 --update-env
```
...
```bash
grep '^QBITTORRENT_HOST_BIND=' /volume1/docker/homelab/hhnas4/qbittorrent/.env
grep '^QBITTORRENT_SERVER_DOMAINS=' /volume1/docker/homelab/hhnas4/qbittorrent/.env
curl -sS -m 6 -I http://127.0.0.1:8080 | sed -n '1,12p'
curl -skI https://qbittorrent.internal.example/ | sed -n '1,12p'
```
