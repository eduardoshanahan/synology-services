...
ssh "${TARGET_HOST}" "cd '${TARGET_DIR}' && ${DOCKER_PREFIX}${DOCKER_BIN} compose pull && ${DOCKER_PREFIX}${DOCKER_BIN} compose up -d && ${DOCKER_PREFIX}${DOCKER_BIN} stop hhnas4-qbittorrent >/dev/null"
...
p.write_text('\\n'.join(out) + '\\n')
PY
${DOCKER_PREFIX}${DOCKER_BIN} start hhnas4-qbittorrent >/dev/null"
