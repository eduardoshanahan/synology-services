...
Shared PHP Adminer UI for the homelab databases that live on `nas-host`.
...
- Deploy path: `/volume1/docker/homelab/nas-host/adminer`
- Compose file: `synology-services/nas-host/adminer/compose.yaml`
...
```bash
cd synology-services/nas-host/adminer
./deploy.sh nas-host
```
...
- The script looks for a private `.env.sops` in `../synology-services-private/
  nas-host/adminer/.env.sops` if you add one later. Keep secrets inside that
  companion repo.
