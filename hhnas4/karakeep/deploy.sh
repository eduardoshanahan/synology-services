...
Examples:
  ./deploy.sh nas-host
  ./deploy.sh nas-host /volume1/docker/homelab/nas-host/karakeep
  ./deploy.sh nas-host --update-env
USAGE
}

TARGET_HOST="nas-host.internal.example"
TARGET_DIR="/volume1/docker/homelab/nas-host/karakeep"
UPDATE_ENV=0
...
ssh "${TARGET_HOST}" "mkdir -p '${TARGET_DIR}' '${TARGET_DIR}/data' '${TARGET_DIR}/meilisearch'"
...
echo "[deploy] karakeep deployed. Verify with:"
