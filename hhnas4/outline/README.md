...
- Preferred: keep the mirrored encrypted source in `../synology-services-private/hhnas4/outline/.env.sops` and use
...
1. Create the shared role/database on `hhnas4/postgres`:

```bash
ssh hhnas4 "cd /volume1/docker/homelab/hhnas4/postgres && sudo /usr/local/bin/docker compose exec -T postgres psql -U postgres -c \"DO \\\$\\\$ BEGIN IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'outline') THEN CREATE ROLE outline LOGIN PASSWORD 'replace-me'; ELSE ALTER ROLE outline WITH LOGIN PASSWORD 'replace-me'; END IF; END \\\$\\\$;\""
ssh hhnas4 "cd /volume1/docker/homelab/hhnas4/postgres && sudo /usr/local/bin/docker compose exec -T postgres psql -U postgres -c \"SELECT 'CREATE DATABASE outline OWNER outline' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'outline')\\\\gexec\""
```

1. Dump from local `outline-postgres` and restore into shared postgres:

```bash
ssh hhnas4 "cd /volume1/docker/homelab/hhnas4/outline && sudo /usr/local/bin/docker compose exec -T outline-postgres pg_dump -U outline -d outline" > /tmp/outline.sql
ssh hhnas4 "cat > /tmp/outline.sql" < /tmp/outline.sql
ssh hhnas4 "cd /volume1/docker/homelab/hhnas4/postgres && sudo /usr/local/bin/docker compose exec -T postgres psql -U postgres -d outline < /tmp/outline.sql"
```
...
```bash
ssh hhnas4 "cd /volume1/docker/homelab/hhnas4/outline && sudo /usr/local/bin/docker compose rm -s -f outline-postgres || true"
ssh hhnas4 "sudo /usr/local/bin/docker volume ls --format '{{.Name}}' | grep '^outline_postgres_data$' && sudo /usr/local/bin/docker volume rm outline_postgres_data || true"
```
...
- PostgreSQL is the primary source of truth. Prefer logical dumps from the
  shared `hhnas4/postgres` stack (`pg_dump`) as the main backup path.
