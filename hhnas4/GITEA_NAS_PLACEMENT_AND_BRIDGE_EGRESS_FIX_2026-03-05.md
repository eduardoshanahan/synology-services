...
## Decision: keep Gitea on `hhnas4` (NAS)

We are keeping Gitea core runtime on Synology `hhnas4`.

### Why

- Gitea currently runs with local persistent state (`/volume1/docker/homelab/hhnas4/gitea/data`).
...
- Gitea (`hhnas4`) -> `smtp-relay.internal.example:2525` -> Gmail upstream.
- Relay log confirms accepted client, queued message, and upstream
  `status=sent` for `contact@eduardoshanahan.com`.
...
```text
/volume1/docker/homelab/hhnas4/ensure-docker-bridge-lan-egress.sh
```
...
```bash
/bin/sh /volume1/docker/homelab/hhnas4/ensure-docker-bridge-lan-egress.sh >> /volume1/docker/homelab/hhnas4/ensure-docker-bridge-lan-egress.log 2>&1
```
