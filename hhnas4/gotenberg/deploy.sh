...
echo "[deploy] gotenberg deployed. Verify with:"
...
echo "         ssh ${TARGET_HOST} \"curl -sS -m 6 -i http://127.0.0.1:\$(grep '^GOTENBERG_PORT=' '${REMOTE_ENV_FILE}' | cut -d= -f2)/health | sed -n '1,12p'\""
