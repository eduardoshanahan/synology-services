...
Examples:
  ./deploy.sh nas-host
  ./deploy.sh nas-host /volume1/docker/homelab/nas-host/paperless
  ./deploy.sh nas-host --update-env
USAGE
}

TARGET_HOST="nas-host.internal.example"
TARGET_DIR="/volume1/docker/homelab/nas-host/paperless"
UPDATE_ENV=0
...
ssh "${TARGET_HOST}" "mkdir -p '${TARGET_DIR}' '${TARGET_DIR}/consume' '${TARGET_DIR}/export'"
...
echo "[deploy] paperless deployed. Verify with:"
...
echo "         ssh ${TARGET_HOST} \"curl -sS -m 6 -i http://127.0.0.1:\$(grep '^PAPERLESS_HOST_PORT=' '${REMOTE_ENV_FILE}' | cut -d= -f2) | sed -n '1,12p'\""
