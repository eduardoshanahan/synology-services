# Paperless-ngx (hhnas4)

Shared Paperless-ngx stack on `hhnas4` using centralized dependencies.
...
```text
/volume1/docker/homelab/hhnas4/paperless
```
...
```bash
ssh hhnas4 'bash -s' < ../ensure-docker-bridge-lan-egress.sh
```
...
```bash
./deploy.sh hhnas4
ssh hhnas4 "sudo -n /usr/local/bin/docker ps --format 'table {{.Names}}\t{{.Status}}' | grep hhnas4-paperless"
```
