...
Dedicated ArchiveBox stack for large local archival storage on `nas-host`.
...
```text
/volume1/docker/homelab/nas-host/archivebox
```
...
```bash
./deploy.sh nas-host.internal.example /volume1/docker/homelab/nas-host/archivebox
```
...
```bash
ssh nas-host "cd /volume1/docker/homelab/nas-host/archivebox && sudo docker compose exec --user archivebox archivebox /bin/bash -lc 'archivebox manage createsuperuser'"
```
