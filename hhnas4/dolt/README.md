...
Shared Dolt SQL server for internal services on `hhnas4`.
...
MySQL service already running on `hhnas4`.

## DNS prerequisite

Create an internal DNS A record before switching clients:

- `dolt.internal.example -> <hhnas4-lan-ip>`
...
```text
/volume1/docker/homelab/hhnas4/dolt
```
...
```bash
./deploy.sh hhnas4.internal.example /volume1/docker/homelab/hhnas4/dolt
```
