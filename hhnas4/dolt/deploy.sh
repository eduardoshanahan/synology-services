...
Examples:
  ./deploy.sh nas-host
  ./deploy.sh nas-host /volume1/docker/homelab/nas-host/dolt
  ./deploy.sh nas-host --update-env
USAGE
}

TARGET_HOST="nas-host.internal.example"
TARGET_DIR="/volume1/docker/homelab/nas-host/dolt"
...
REMOTE_SERVER_DIR="${TARGET_DIR}/server"
...
ssh "${TARGET_HOST}" "mkdir -p '${TARGET_DIR}' '${REMOTE_SERVER_DIR}'"

cat "${SRC_DIR}/compose.yaml" | ssh "${TARGET_HOST}" "cat > '${REMOTE_COMPOSE_FILE}'"
cat "${SRC_DIR}/server/config.yaml" | ssh "${TARGET_HOST}" "cat > '${REMOTE_SERVER_DIR}/config.yaml'"
...
echo "[deploy] dolt deployed. Verify with:"
...
echo "         ssh ${TARGET_HOST} \"curl -sS http://127.0.0.1:\${DOLT_METRICS_PORT:-11228}/metrics | sed -n '1,20p'\""
