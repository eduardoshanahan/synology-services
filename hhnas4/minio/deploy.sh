...
Examples:
  ./deploy.sh nas-host
  ./deploy.sh nas-host /volume1/docker/homelab/nas-host/minio
  ./deploy.sh nas-host --update-env
USAGE
}

TARGET_HOST="nas-host.internal.example"
TARGET_DIR="/volume1/docker/homelab/nas-host/minio"
UPDATE_ENV=0
...
echo "[deploy] minio deployed. Verify with:"
...
echo "         ssh ${TARGET_HOST} \"curl -sS -m 6 -i http://127.0.0.1:\$(grep '^MINIO_API_PORT=' '${REMOTE_ENV_FILE}' | cut -d= -f2)/minio/health/live | sed -n '1,12p'\""
