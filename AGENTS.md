...
`hhnas4/` is the active host directory and the main operational focus in this
repo.
...
- keep real hostnames such as `hhnas4`, real deployment paths, real LAN IPs,
  and other operator-private values in `../synology-services-private/`
...
- if a public file shows a concrete-looking path or hostname, treat it as an
  example until the private companion repo and live host confirm that the file
  is intentionally meant to carry real values

The same rule applies to deployment paths and other operational values that may
look concrete in the public repo. Paths such as
`/volume1/docker/homelab/nas-host/...`, example hostnames, example ports, and
similar values should be treated as public documentation examples unless they
have been confirmed against the private companion repo and the live host. The
real deployment values and operator-maintained continuity notes live in
...
- `hhnas4/archivebox/deploy.sh`
  - seeds the Docker volume before startup for Chrome profile and optional
    cookies file
- `hhnas4/postgres/deploy.sh` and `hhnas4/dolt/deploy.sh`
